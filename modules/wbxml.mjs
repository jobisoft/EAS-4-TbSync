...
export function decodeWBXML(bytes) {
  ...
  while (pos < bytes.length) {
    ...
    switch (token) {
      ...
      case 0x03: {
        // STR_I: inline string, NUL-terminated
        let end = pos + 1;
      ...
      case 0xc4:
        throw new Error(
          `wbxml: global token 0x${token.toString(16)} not supported`,
        );
...
export function createWBXML(initialNamespace = "") {
  ...
  const tokenFor = (name) => {
    ...
    if (!page || !(name in page)) {
      throw new Error(
        `wbxml: unknown tag '${name}' in codepage '${NAMESPACES[codepage]}'`,
      );
    }
    return page[name];
  };
  ...
  return {
    switchpage(name) {
      ...
      if (idx === undefined)
        throw new Error(`wbxml: unknown namespace '${name}'`);
      codepage = idx;
      bytes.push(0x00, idx);
    },
